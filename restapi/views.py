...
def getMyTeamId(request):
    ...
    sorter = queryprms.get('sorter', "asc")
    cursor = connection.cursor()
    cursor.execute('select t.id, u.fname, u.lname, u.role, u.cid, u.mobile, u.email, u.username, u.rank, u.election_option from (select id, teamuser_id from tb_team_members where candidate_id = ' + userid + ' and del_flag = 0) as t join (select * from tb_users where del_flag = 0) as u on t.teamuser_id = u.id where (fname rlike "' +
    ...
    json_data = []
    for obj in objs:
        json_data.append({"id": obj[0], "fname": obj[1], "lname": obj[2], "role": obj[3], "cid": obj[4],
                         "mobile": obj[5], "email": obj[6], "username": obj[7], "rank": obj[8], "election_option": obj[9]})
...
def getMyCandidateId(request):
    ...
    sorter = queryprms.get('sorter', "asc")
    cursor = connection.cursor()
    cursor.execute('select t.id, u.fname, u.lname, u.role, u.cid, u.mobile, u.email, u.username, u.rank, u.election_option from (select id, candidate_id from tb_team_members where teamuser_id = ' + userid + ' and del_flag = 0) as t join (select * from tb_users where del_flag = 0) as u on t.candidate_id = u.id where (fname rlike "' +
                   keyword + '" or lname rlike "' + keyword + '" or cid rlike "' + keyword + '" or mobile rlike "' + keyword + '") order by ' + filter + ' ' + sorter + ' limit ' + str(limit) + ' offset ' + str(limit * (pagenum - 1)), None)
    objs = cursor.fetchall()
    json_data = []
    for obj in objs:
        json_data.append({"id": obj[0], "avatar": obj[1], "fname": obj[1], "lname": obj[2], "role": obj[3], "cid": obj[4],
                         "mobile": obj[5], "email": obj[6], "username": obj[7], "rank": obj[8], "election_option": obj[9]})
